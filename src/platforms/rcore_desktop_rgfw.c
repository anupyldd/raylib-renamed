...
void rl_ToggleBorderlessWindowed(void)
{
    ...
    if (platform.window != NULL)
        TRACELOG(LOG_WARNING, "rl_ToggleBorderlessWindowed() after window creation not available on target platform");
}
...
void rl_SetWindowState(unsigned int flags)
{
    ...
    {
        TRACELOG(LOG_WARNING, "rl_SetWindowState() - FLAG_WINDOW_TRANSPARENT is not supported on PLATFORM_DESKTOP_RGFW");
    }
    ...
    {
        TRACELOG(LOG_WARNING, "rl_SetWindowState() - FLAG_WINDOW_MOUSE_PASSTHROUGH is not supported on PLATFORM_DESKTOP_RGFW");
    }
